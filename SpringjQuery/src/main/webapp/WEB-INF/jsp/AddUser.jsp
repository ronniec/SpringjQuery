...
function doAjaxPost() {  
	...
	alert(2);
	  ...
	  var education = $('#education').val();
	   ...
	   alert('in ajax call' +"${pageContext.request.contextPath}");
	    $.ajax({ 
		 type: "POST",  
	    url:  "${pageContext.request.contextPath}"+"/AddUser.htm",  
	    data: "name=" + name + "&education=" + education,  
	    success: function(response){
	      // we have the response 
	      alert("in response"+response.status);
	      if(response.status == "SUCCESS"){
	    	  alert('respose'+response.status)
	    	  userInfo = "<ol>";
